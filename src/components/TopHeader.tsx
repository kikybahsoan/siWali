  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        {/* Role Badge & Auth Action */}
        {isAdmin ? (
          <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-300 px-2 sm:px-2.5 py-1 rounded-lg text-emerald-800 text-xs font-bold shadow-xs">
            <Lock className="w-3.5 h-3.5 text-emerald-600" />
            <span className="hidden sm:inline">Admin (Wali Kelas)</span>
            <button
              onClick={onLogoutAdmin}
              className="ml-1 p-1 hover:bg-emerald-100 rounded text-emerald-700 hover:text-emerald-900 transition-colors flex items-center gap-1"
              title="Keluar dari Mode Admin ke Mode Tamu"
            >
              <LogOut className="w-3 h-3" />
              <span className="hidden md:inline text-[11px] font-medium">Keluar</span>
            </button>
          </div>
        ) : (
          <div className="flex items-center gap-1.5 bg-slate-100 border border-slate-200 px-2 sm:px-2.5 py-1 rounded-lg text-slate-700 text-xs font-medium shadow-xs">
            <Eye className="w-3.5 h-3.5 text-slate-500" />
            <span className="hidden sm:inline">Mode Tamu (Lihat)</span>
            <button
              onClick={onOpenAuthModal}
              className="ml-1 px-2 py-0.5 bg-blue-900 hover:bg-blue-800 text-white rounded font-bold text-[11px] transition-colors flex items-center gap-1 shadow-xs"
              title="Masuk sebagai Wali Kelas untuk mengedit data"
            >
        ...
        {onQuickNewAction && (
          <button
            ...
            onClick={isAdmin ? onQuickNewAction : onOpenAuthModal}
            className="bg-blue-900 hover:bg-blue-800 px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium text-white shadow-sm transition-all flex items-center gap-1.5 active:scale-95"
            title={isAdmin ? 'Tambah data baru' : 'Masuk Admin untuk menambah data'}
          >
        ...
        <button
          ...
          title={isAdmin ? 'Pengaturan Profil Guru & Kop Surat' : 'Masuk Admin untuk ubah Pengaturan'}
        >
